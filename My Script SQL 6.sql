...
#----------------------------------------------------------------------14/11/23------------------------------------------------------------------------------------------------
#Aggregate Funtion 
#Aggregate function sum average Max min count distint count ( describe all information )
...
#----------------------------------------------------not Between---------------------------------------------------------
 ...
 order by orderDate;
 #----------------------------------------------------------------------------------
 # Between  not used for text Data
 Select customername ,Country from 
 customers
 where country between "Uk" And "Canada"
 order by country;
 

 #------------------------------------------------------------------TCL----------
 #Transaction Control Languages, Commad
 use Ashwini;
 ...
 savepoint insert_Ashwini;
 
 update Students
 set Age =30
 where Roll_No=3;
 savepoint update_Ashwini;
 
 delete from Students
 where Roll_No=1;
 savepoint delete_Ashwini;
 
 insert INTO students
 value(4,"TOYO", 25) ,(5,"priyanka", 27);
 
 commit; #remove All record After commit
 rollback to update_Ashwini; #remove the record ,like truncate
